...
- points_model.py  (PTS-only, legacy)
...
class TestFeatureEngineering:
    def test_engineer_features_adds_rolling_cols(self, db):
        from models.nba.points_model import _engineer_features
        ...
        assert "pts_last5_avg" in result.columns
        assert "pts_last10_avg" in result.columns
        assert "min_last5_avg" in result.columns
    ...
    def test_home_game_flag_correct(self, db):
        ...
        assert (home_rows["home_game"] == 1).all()

        away_rows = result[result["matchup"].str.contains("@", na=False)]
        assert (away_rows["home_game"] == 0).all()

    def test_rolling_avg_does_not_leak_same_game(self, db):
        """pts_last5_avg should use shift(1) so it doesn't include same-game pts."""
        from models.nba.points_model import _engineer_features
        ...
        result = _engineer_features(df).dropna(subset=["pts_last5_avg", "pts"])

        # First row after shifting has NaN — already dropped. The remaining
        # rows' last5_avg should NOT equal the same row's pts (unless coincidence).
        # At minimum, test that same-row pts is not copied directly to avg.
        for _, row in result.head(5).iterrows():
            # The avg should be the mean of PRIOR games, not just the current row
            assert not np.isnan(row["pts_last5_avg"])


class TestTrainPredict:
    def test_train_saves_model(self, db, tmp_path, monkeypatch):
        """Training should produce a .joblib file."""
        import joblib
        from pathlib import Path

        model_path = tmp_path / "points_model.joblib"
        encoder_path = tmp_path / "team_encoder.joblib"

        monkeypatch.setattr("models.nba.points_model.MODEL_PATH", model_path)
        monkeypatch.setattr("models.nba.points_model.ENCODER_PATH", encoder_path)

        _seed_game_logs(30)
        from models.nba.points_model import train
    ...
    def test_train_handles_no_data(self, db, tmp_path, monkeypatch, capsys):
        """Training with no data should log an error, not raise."""
        model_path = tmp_path / "points_model.joblib"
        encoder_path = tmp_path / "team_encoder.joblib"
    ...
    def test_model_predicts_reasonable_pts(self, db, tmp_path, monkeypatch):
        """Predictions should be in a plausible NBA scoring range (5-60)."""
        import joblib

        model_path = tmp_path / "pts_model.joblib"
        encoder_path = tmp_path / "team_encoder.joblib"
        ...
        _seed_game_logs(30)
        from models.nba.points_model import train, _engineer_features, _encode_opponents, FEATURE_COLS
        ...
        assert all(5 <= p <= 60 for p in preds), f"Out-of-range prediction: {preds.min():.1f} - {preds.max():.1f}"
