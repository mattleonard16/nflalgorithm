...
def filter_stale_snapshots(
    ...
    Args:
        ...
        max_age_hours: Freshness window before kickoff; must be positive.
            Defaults to a full week: the production run happens days before
            kickoff, and a shorter window emptied the card on every run
            before Friday. Fetch-time freshness is enforced separately by
            the odds validator.
